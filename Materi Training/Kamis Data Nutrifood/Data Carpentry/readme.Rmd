...
Sekarang kita akan mencoba untuk mengolah _unstructured data_ menjadi informasi atau _insight_.
...
_Nah_, saya sudah mendapatkan data yang saya butuhkan. Sekarang saya akan membuat _error bar_.

```{r,echo=FALSE,message=FALSE}
library(ggpubr)

sedan.data %>% group_by(tahun) %>% summarise(rata = mean(harga),stdev = sd(harga),n = n()) %>%
  mutate(stdev = ifelse(is.na(stdev),0,stdev)) %>%
  ggplot(aes(x = as.factor(tahun), y = rata)) + 
