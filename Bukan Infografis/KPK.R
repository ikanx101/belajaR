...
windows()
png('big fish KPK.png',width = 1500,height = 750,res=100)
data %>% filter(jabatan %in% target) %>%
  ggplot(aes(x=(tahun),y=value)) + geom_line(aes(linetype=jabatan,color=jabatan),size=1) + 
  ...
  labs(color='Jabatan',linetype='Jabatan') +
