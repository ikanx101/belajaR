...
data = data %>% mutate(total_domestic_box_office = gsub('\\,','',total_domestic_box_office),
  ...
  filter(no_of_movies>=10)

new_data = data %>% mutate(no_of_movies = cut(no_of_movies,6,labels = c(1:6)),total_domestic_box_office = cut(total_domestic_box_office,6,labels = c(1:6)),total_worldwide_box_office = cut(total_worldwide_box_office,6,labels = c(1:6)))
```

Sekarang, saya hanya akan memilih perusahaan yang minimal telah memproduksi 10 film lalu saya buat _scatter plot_ dari data tersebut. Didapat hanya `468` buah perusahaan. Sumbu `x` akan saya isi dengan `no_of_movies` dan sumbu `y` akan saya isi dengan `total_worldwide_box_office`, sementara _size_ dari _point_ tergantung dari besarnya `total_domestic_box_office`.
...
Ternyata disarankan ada tiga buah _clusters_ yang terbentuk. Mari kita _generate_ _clusters_ nya!

## 8-means clustering

```{r,echo=FALSE}
set.seed(123)
final = kmeans(new_data,3,nstart=10)
centeroids = round(final$centers,2)
centeroids
data.frame(cluster=paste('cluster',c(1:3)),
                             n=final$size) %>%
  ggplot(aes(x=cluster,y=n)) +
  geom_col(color='black',fill='steelblue',alpha=.4) +
  geom_label(aes(label=n)) +
  labs(title = 'Berapa anggota dari masing-masing cluster?')
```


final$cluster
new_data$cluster = final$cluster
head(new_data)